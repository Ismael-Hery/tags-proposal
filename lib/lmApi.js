...
var apiBaseUrl = 'http://www-org.lemonde.fr/api/2/document/element/';
...
exports.getArticles = function(articleIDs, articlesCB) {
   var apiURL = apiBaseUrl + '?id[]=' + articleIDs.join('&id[]=') + '&complete=true';
   request.get(apiURL).end(function (res) {
      articlesCB(res.body);
   });
};
...
exports.lmApi = function (query, options, articleCB, endCB) {
   ...
   request.get(baseUrl).query({count:true}).query(options).end(function (res) {
      if (res.ok) {
